var game = new Phaser.Game(1200,900, Phaser.AUTO, 'phaser-demo', {preload: preload, create: create, update: update, render: render});
...
function create() {
    ...
    starfield = game.add.tileSprite(0, 0, 1200, 900, 'starfield');
    ...
    //  The hero!
    player = game.add.sprite(600, 720, 'ship');
    player.health = 100;
    ...
    // shield
    shield = game.add.sprite(480, 590, 'shield');
    ...
    shield.alpha = 0;
    game.add.tween(shield).to( { alpha: 1 }, 1000, Phaser.Easing.Linear.None, true, 0, 500, true);
    
    player.addChild(shield);
    shield.visible = 0;
    
    // rockets
    rocket1 = game.add.sprite(480, 590, 'rocket');
    rocket1.anchor.setTo(0.5, 0.5);
    game.physics.enable(rocket1, Phaser.Physics.ARCADE);
    rocket1.body.collideWorldBounds = true; 
    game.physics.arcade.enableBody(rocket1);
    rocket1.y = 15;
    rocket1.x = -24;
    
    rocket2 = game.add.sprite(480, 590, 'rocket');
    rocket2.anchor.setTo(0.5, 0.5);
    game.physics.enable(rocket2, Phaser.Physics.ARCADE);
    rocket2.body.collideWorldBounds = true; 
    game.physics.arcade.enableBody(rocket2);
    rocket2.y = 15;
    rocket2.x = 24;
    
    
    player.addChild(rocket1);
    player.addChild(rocket2);
   
    
    
    //shield.body.setSize(shield.width / 5, shield.height / 4);
    ...
    shieldKey = game.input.keyboard.addKey(Phaser.Keyboard.F);
    ...
    blasts.createMultiple(30, 'blast');
    blasts.setAll('anchor.x', 0.5);
    blasts.setAll('anchor.y', 0.5);
    ...
    shieldstatus = game.add.bitmapText(game.world.width - 550, 10, 'spacefont', '' + (shieldValue) +'%', 50);
    shieldstatus.render = function () {
        shieldstatus.text = 'Shield: ' + (shieldValue) +'%';
    };
    shieldstatus.render();
...
function update() {
   ...
   /* if (cursors.left.isDown)
    ...
    if (player.x > game.width - 150) {
        player.x = game.width - 150;
        player.body.acceleration.x = 0;
        shield.anchor.x = 0.5;
        shield.x = 0;
    }
    ...
    game.physics.arcade.overlap(shield,blueEnemies, shieldEnemy, null, this);
    game.physics.arcade.overlap(shield,greenEnemies, shieldEnemy, null, this);
...
function sendMisile(){
    if(rocketFired == false){
        switch (activeRocket){
            case 1:
                activeRocket = 2;
                lunchMisilie(rocket1 , player.x - 24);
            break;
            case 2:
                activeRocket = 0;
                lunchMisilie(rocket2 , player.x + 24);
...
function lunchMisilie(rocket, x){
    ...
    if(rocketFired == false){
        ...
        var Missile = function(game, x, y) {
            ...
            this.SMOKE_LIFETIME = 4000; // milliseconds
            ...
            this.smokeEmitter.setScale(0.8, 0, 0.8, 0, 3000);
        ...
        Missile.prototype.update = function() {
            ...
            if (this.rotation !== targetAngle) {
                ...
                if(this.y < this.targetY + 100 && (this.x > this.targetX-50 && this.x < this.targetX + 50) ){
                    var x1 = this.x;
                    var x2 = this.x2;
                   ...
                   /* var explosion = explosions.getFirstExists(false);
                    ...
                    explosion.scale.x = 5;
                    explosion.scale.y = 5;
                    var animation = explosion.play('explosion', 50, false, true);
                    animation.onComplete.addOnce(function(){
                        explosion.scale.x = beforeScaleX;
                        explosion.scale.y = beforeScaleY;
                        explosion.alpha = beforeAlpha;
                    ...
                    blueEnemies.forEach(function(enemy) {
                        if(enemy.alive){
                            var explosion = explosions.getFirstExists(false);
                            explosion.reset(enemy.x + enemy.body.halfWidth, enemy.body.y + enemy.body.halfHeight);
                            explosion.alpha = 0.7;
                            explosion.play('explosion', 30, false, true);
        ...
        game.add.existing(
            new Missile(this.game, XX, player.y)
        );
...
function shieldEnemy(shield,enemy){
    if(shieldIsOn){
        enemy.kill();
        var explosion = explosions.getFirstExists(false);
        explosion.reset(player.body.x + player.body.halfWidth, player.body.y + player.body.halfHeight);
